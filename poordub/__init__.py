...
class PcmAudio:
    ...
    * Extracting segments: Slice by the desired number of milliseconds,
    ...
    class Params(namedtuple("pcm_params", "nchannels sampwidth framerate nframes")):
        ...
        def __init__(self, *args, **kw):
            ...
            if self.sampwidth not in (1, 2, 3, 4):
                raise PcmValueError(f"Illegal sample width: {self.sampwidth}")
    ...
    def __add__(self, other):
        ...
        if other.params.nframes == 0:
            return self
        if self.params.nframes == 0:
            return other
    ...
    def to_sample_width(self, sampwidth):
        ...
        if sampwidth not in (1, 2, 3, 4):
            raise PcmValueError(f"Illegal sample width: {sampwidth}")
    ...
    def to_framerate(self, framerate):
        ...
        if framerate not in self.Params.FRAME_RATES:
            raise PcmValueError(f"Illegal frame rate: {framerate}")
        ...
        return self.__class__(
            (
                ...
                self.params.sampwidth,
                framerate,
                int(len(frames) / self.params.frame_size),
            ),
            frames,
        )
    ...
    def normalize(self, to=-0.1):
        ...
        peak = self._max()
        if peak == 0:
            return self  # silence cannot be normalized
        if to > 0:
            raise PcmValueError("Negative dB value expected")
...
if __name__ == "__main__":  # Demo code
    ...
    parser.add_argument(
        "-m", "--mono", action="store_true", help="Convert output to mono"
    )
    parser.add_argument("-g", "--gain", type=float, help="Add gain to output (in dB)")
    parser.add_argument(
        "-%",
        "--volume",
        dest="gain",
